...
def normalized(payload: dict) -> dict:
    ...
    result = dict(payload)
    result.setdefault("cwd", os.environ.get("DEVIN_PROJECT_DIR") or "")
    result["provider"] = "devin"
    if canonical := CANONICAL_TOOLS.get(result.get("tool_name") or ""):
        result["tool_name"] = canonical
    return result


def devin_result(event: str, result: dict) -> dict:
    """Convert the shared Claude-shaped decision into Devin's native output contract."""
    ...
    if message := result.get("systemMessage"):
        return {
            "hookSpecificOutput": {
                "hookEventName": NATIVE_EVENTS.get(event, event),
...
def native(event: str, payload: dict) -> int:
    ...
    try:
        result = verification_session.dispatch("devin", event, normalized_payload)
    except (OSError, ValueError, TypeError, KeyError) as error:
        result = verification_session.failure(event, normalized_payload, error)
    print(json.dumps(devin_result(event, result), ensure_ascii=True, separators=(",", ":")))
    return 0


def audit(kind: str, payload: dict) -> int:
    context = normalized(payload)
    entry = audit_log.record(kind, context, "devin")
...
def lint(payload: dict) -> int:
    """Report shared quality diagnostics using Devin's PostToolUse JSON contract."""
...
def main(arguments: list[str]) -> int:
    ...
    if not valid or any(value in {"-h", "--help"} for value in rest):
        print(
            "Usage: hook_adapter.py <native session-start/pre-tool-use/stop/session-end"
            "|audit tool/compaction|shell allowed/forbidden/git/commit|lint|harness"
    ...
    try:
        ...
        if not isinstance(payload, dict):
            payload = {}
        cwd = payload.get("cwd") or os.environ.get("DEVIN_PROJECT_DIR")
        if cwd:
            os.chdir(cwd)
