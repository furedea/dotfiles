...
def manifest_path() -> Path:
    return Path(os.environ.get("AGENT_HARNESS_HERMES_MANIFEST", str(Path.home() / ".hermes/hooks.json")))
...
def hermes_result(event: str, result: dict) -> dict:
    """Convert the shared Claude-shaped decision into Hermes's blocking contract."""
...
def native(event: str, payload: dict) -> int:
    ...
    try:
        result = verification_session.dispatch("hermes", event, normalized_payload)
    except (OSError, ValueError, TypeError, KeyError) as error:
        result = verification_session.failure(event, normalized_payload, error)
    print(json.dumps(hermes_result(event, result), ensure_ascii=True, separators=(",", ":")))
    return 0


def audit(kind: str, payload: dict) -> int:
    context = normalized(payload)
    entry = audit_log.record(kind, context, "hermes")
