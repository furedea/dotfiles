...
class Report:
    ...
    def record_failure(
        ...
        if not self.attempted_logs:
            ...
            self.directory = verification_logs.prepare_directory(self.root)
        if self.directory is None:
            return
        self.records += 1
        metadata = f"runner: {label}\nstatus: {status}\nscope: {scope}\ncwd: {self.root}\ncommand: {command}\nduration: {duration}\n"
        try:
            verification_logs.write_failure(self.directory, self.records, metadata, output)
        except OSError:
            self.directory = None
    ...
    def notification(self, skipped_reason: str = "") -> dict[str, str]:
        ...
        if self.failed:
            lines = [
                ...
                *self.lines,
                *self.errors,
                verification_logs.finish(self.directory),
            ]
...
def verify(invocation: verification_selection.Invocation, report: Report) -> None:
    ...
    if timeout is None:
        ...
        return
    budget = os.environ.get("RUN_RELATED_TESTS_TIMEOUT_SECONDS", "300" if label == "bats" else "120")
...
def main() -> int:
    ...
    if sys.argv[1:]:
        print("Usage: run_related_tests.py < hook-input.json", file=sys.stderr)
