...
```bash
slug=$(<command-dir>/scripts/branch_name.py "<conventional message>" --avoid-existing)
```

Resolve `<command-dir>` as in `references/hunk_split.md`. The script drops the optional commit scope and produces `<type>/<kebab-subject>`; if the slug already exists locally or on `origin`, it appends `-2`, `-3`, … and prints the unused name.
