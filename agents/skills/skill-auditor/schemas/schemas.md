  ...
  "skills": [
    {
      ...
      "name": "string",
      "scope": "global | project-local | catalog",
    ...
    "skill_reports": [
        {
            ...
            "stats": {
                ...
                "accuracy": "number — 0.0 to 1.0 | null when unassessable"
            },
            "incidents": [
                {
                    "session_id": "string",
...
For Codex, transcript `skills_loaded_after` includes unverified read attempts, excluding
known failures. Use `skill_read_evidence` to inspect command success; it does not prove
complete reading or automatic invocation. `injected_skills` records available injection
names separately. Session `user_turn_basis` distinguishes user events from response-item
fallback, which may include replayed history. `historical_visibility: unknown` prohibits
false-negative inference from the current inventory alone.

Accuracy fields may be `null` when unassessable. Preserve this through merge and report
generation. Inventory `scope: catalog` entries are source/cache candidates, not installed
skills. Current global/project-local entries are not proof of historical availability.
Inventory token totals are estimates, not measured context usage.
