...
# Bash(rm:*) -> rm:* -> rm * (legacy `:` separator converted to space for glob matching)
# Bash(rm *)  -> rm *  (modern format, no colon, unchanged)
deny_patterns=$(jq -r '.permissions.deny[] | select(startswith("Bash(")) | .[5:-1] | gsub(":"; " ")' "$settings_file" 2>/dev/null)
