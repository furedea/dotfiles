...
\$XDG_CONFIG_HOME/dotfiles/bootstrap_server.env:
...
readonly HOST_ENV_FILE="${BOOTSTRAP_SERVER_ENV:-$CONFIG_DIR/bootstrap_server.env}"
...
MACHINE="${BOOTSTRAP_SERVER_MACHINE:-$(uname -m)}"
readonly MACHINE
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bootstrap_server.XXXXXX")"
readonly WORK_DIR
...
function write_env_snippet() {
  {
    printf '%s\n' \
      '# Generated by scripts/agents/bootstrap_server.sh; source this file from the login shell.' \
      "case \":\$PATH:\" in" \
      "  *\":$BIN_DIR:\"*) ;;" \
