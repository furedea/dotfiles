...
def test_executable_entry_point_renders_the_single_line_snapshot(isolated_project: Path) -> None:
    ...
    assert "Ctx:" in lines[0] and "25%" in lines[0]
    assert "5h:" not in lines[0]


def test_git_repository_contributes_no_branch_segment() -> None:
    """A Git working directory must not add a segment beyond the directory and model."""
    script = REPO_ROOT / "agents/claude/statusline/statusline.py"
    result = subprocess.run(
        [str(script)],
        input=json.dumps({"model": {"display_name": "Opus 4.6"}, "cwd": str(REPO_ROOT)}),
        text=True,
        capture_output=True,
        timeout=10,
        check=False,
    ...
    assert result.returncode == 0
    assert re.sub(r"\x1b\[[0-9;]*m", "", result.stdout).count("│") == 2
...
def test_missing_model_still_renders_the_context_segment() -> None:
    rendered = statusline.render({"model": {}, "context_window": {"used_percentage": 50}}, 0)
    assert len(rendered.splitlines()) == 1
    assert "Ctx:" in rendered and "50%" in rendered


def test_long_directory_is_reduced_to_the_current_name() -> None:
    rendered = statusline.render({"cwd": "/a/b/c/d/e"}, 0)
    assert "e" in rendered and "c/d/e" not in rendered
...
def test_rate_limits_without_reset_time_still_show_usage(window: str, label: str, percentage: int) -> None:
    rendered = statusline.render({"rate_limits": {window: {"used_percentage": percentage, "resets_at": None}}}, 0)
    assert label in rendered and f"{percentage}%" in rendered
