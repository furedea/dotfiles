...
install_policy_fixture() {
  local _home="$1"

  mkdir -p "$_home/.claude/hooks/rules"
...
*** End Patch'
  ...
  run env HOME="$_tmp" CLAUDE_PROJECT_DIR="$_tmp" bash -c "printf '%s' '$_input' | '$HOOK' harness"
  [ "$status" -eq 0 ]
  [ -z "$output" ]
}
...
@test "resolves shared policy from the harness root" {
  ...
  run env HOME="$_home" AGENT_HARNESS_ROOT="$_harness_root" \
    CLAUDE_PROJECT_DIR="$_home" bash -c "printf '%s' '$_input' | '$HOOK' harness"

  [ "$status" -eq 0 ]
  [ -z "$output" ]
}

@test "blocks when shared policy is missing" {
  local _tmp
  _tmp="$(mktemp -d "$BATS_TEST_TMPDIR/codex.XXXXXX")"
  ...
  run env HOME="$_tmp" bash -c "printf '%s' '$_input' | '$HOOK' harness"
  [ "$status" -eq 2 ]
  [[ "$output" == *"BLOCKED"* ]]
}
