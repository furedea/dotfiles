...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            2,
            [],
            id="blocks --force-with-lease without --force-if-includes",
        ),
        pytest.param(
        ...
        pytest.param(
            "git push --force-with-lease --force-if-includes origin feature/foo",
            0,
            [],
            id="allows leased force pushes that include the remote tip",
        ),
        pytest.param(
            "git push --force-with-lease --force-if-includes origin main",
            2,
            ["main"],
            id="blocks leased force pushes to protected branches",
        ),
        pytest.param(
            "git push --force --force-if-includes origin feature/foo",
            2,
            [],
            id="blocks bare --force even with --force-if-includes",
        ),
        ...
        pytest.param("git rm foo.py", 0, [], id="allows git rm (HEAD can restore the file)"),
        pytest.param("git rm --cached foo.py", 0, [], id="allows git rm --cached"),
        pytest.param("git rm -r dir/", 0, [], id="allows git rm -r"),
        pytest.param("git rm -f foo.py", 2, [], id="blocks git rm -f (discards local modifications)"),
        pytest.param("git mv -f a.py b.py", 2, [], id="blocks git mv -f (overwrites the target)"),
        ...
        pytest.param("git stash drop", 0, [], id="leaves git stash drop to the ask rule"),
        pytest.param("git stash clear", 0, [], id="leaves git stash clear to the ask rule"),
        ...
        pytest.param("git worktree remove ../foo", 0, [], id="allows git worktree remove (refuses dirty trees)"),
        pytest.param("git worktree remove --force ../foo", 2, [], id="blocks git worktree remove --force"),
        pytest.param("git worktree remove -f ../foo", 2, [], id="blocks git worktree remove -f"),
        pytest.param("git worktree prune", 0, [], id="allows git worktree prune"),
        ...
        pytest.param('bash -c "git rm -f foo.py"', 2, [], id="blocks bash -c with git rm -f"),
