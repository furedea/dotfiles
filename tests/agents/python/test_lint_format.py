"""Quality planning and notifications are independent of shell implementation details."""
...
quality = load_script_module("agents/hooks/lint_format.py", "quality")
...
def test_nearest_project_marker_is_found_from_nested_directories(tmp_path: Path, marker: str) -> None:
    ...
    assert quality.project_root(nested, ("pyproject.toml", "uv.lock", "Cargo.toml")) == directory


def test_missing_project_marker_has_no_invented_root(tmp_path: Path) -> None:
    assert quality.project_root(tmp_path, ("nonexistent_marker_file.xyz",)) is None
...
def test_language_plans_preserve_the_configured_quality_tools(
    ...
    assert tuple(step.arguments[0] for step in quality.plan(kind, tmp_path / filename)) == tools


def test_python_project_uses_its_frozen_uv_tools(tmp_path: Path) -> None:
    (tmp_path / "pyproject.toml").touch()
    steps = quality.plan("py", tmp_path / "src" / "file.py")
...
def test_python_outside_a_project_uses_available_ruff(tmp_path: Path) -> None:
    assert all(step.arguments[0] == "ruff" for step in quality.plan("py", tmp_path / "file.py"))


def test_missing_tool_reports_target_and_prevents_later_steps(tmp_path: Path, mocker: MockerFixture) -> None:
    mocker.patch.object(quality.shutil, "which", return_value=None, autospec=True)
    execute = mocker.patch.object(quality.subprocess, "run", autospec=True)
    target = tmp_path / "file with spaces.sh"
    assert quality.check_file("sh", target) == (
...
def test_successful_steps_are_silent_even_with_output(tmp_path: Path, mocker: MockerFixture) -> None:
    mocker.patch.object(
        quality.subprocess, "run", return_value=subprocess.CompletedProcess([], 0, b"all good\n"), autospec=True
    )
    assert quality.run_step(quality.Step("example lint", ("example",)), tmp_path / "file") == ""


def test_failure_without_diagnostics_preserves_exit_status(tmp_path: Path, mocker: MockerFixture) -> None:
    mocker.patch.object(quality.subprocess, "run", return_value=subprocess.CompletedProcess([], 3, b""), autospec=True)
    target = tmp_path / "source.sh"
    assert quality.run_step(quality.Step("example lint", ("example",)), target) == (
...
def test_context_preserves_event_tool_label_and_multiline_diagnostics() -> None:
    ...
    assert json.loads(json.dumps(quality.context(message))) == {
...
def test_missing_file_is_not_treated_as_a_successful_check(
    ...
    mocker.patch.object(
        quality.sys, "stdin", io.StringIO(json.dumps({"tool_input": {"file_path": str(tmp_path / "missing")}}))
    )
    assert quality.main(["sh"]) == 1
    assert "File not found" in capsys.readouterr().err


def test_empty_input_does_not_execute_quality_tools(mocker: MockerFixture, capsys: pytest.CaptureFixture[str]) -> None:
    mocker.patch.object(quality.sys, "stdin", io.StringIO('{"tool_input":{}}'))
    execute = mocker.patch.object(quality.subprocess, "run", autospec=True)
    assert quality.main(["sh"]) == 0
    execute.assert_not_called()
...
def test_formatted_replacement_preserves_file_permissions(tmp_path: Path) -> None:
    ...
    target.chmod(0o640)
    quality.replace_formatted_file(target, b"after")
